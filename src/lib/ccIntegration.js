// ── CONSTANT CONTACT INTEGRATION (v1.0) ─────────────────────────────────────
// OAuth 2.0 Authorization Code flow + Contact sync to CC lists.
// Tokens stored in localStorage. Client secret in .env (local dev only).
// For production: move token exchange to a Supabase Edge Function.

const CC_CLIENT_ID     = import.meta.env.VITE_CC_CLIENT_ID;
const CC_CLIENT_SECRET = import.meta.env.VITE_CC_CLIENT_SECRET;
...
// ── STEP 1: Open CC OAuth in current window ───────────────────────────────────
...
// Pushes an array of CRM leads into a CC contact list.
// Uses CC's bulk "add_list_memberships" activity for efficiency.
// Returns: { accepted, rejected, activityId }
export const ccSyncLeads = async (leads, listId) => {
  if (!listId) throw new Error('No CC list selected');

  // Build CC contact payload from CRM lead schema
  ...
  // CC bulk import: POST /activities/contacts
