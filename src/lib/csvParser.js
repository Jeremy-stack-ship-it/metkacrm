...
export function autoDetectMapping(lowerHeaders) {
  ...
  return {
    ...
    cellPhone:   fi(["cellphone","cell_phone","mobile","mobilephone","mobile_phone"]),
    homePhone:   fi(["homephone","home_phone"]),
    workPhone:   fi(["workphone","work_phone"]),
    ...
    homeValue:        fi(["homevalue","home_value","propertyvalue","property_value"]),
    householdIncome:  fi(["householdincome","household_income","income","annualincome","annual_income"]),
    reqCoverage:      fi(["requestedcoverageamount","requested_coverage_amount","requestedcoverage","coverage_amount","coverageamount"]),
    spouseName:       fi(["spousename","spouse_name","co_applicant","coapplicant"]),
    spouseAge:        fi(["spouseage","spouse_age"]),
    spouseDob:        fi(["spousedob","spouse_dob","spouse_birthday"]),
    dependents:       fi(["dependents","numberofchildren","number_of_children","children"]),
    leadLevelAlias:   fi(["leadlevelalias","lead_level_alias","leadlevel","lead_level"]),
    beneficiary:      fi(["leadbeneficiaryrelationship","beneficiary_relationship","beneficiaryrelationship","beneficiary"]),
    reqCoverageRange: fi(["requestedcoverageamountrange","requested_coverage_amount_range","coveragerange"]),
  };
}
...
export function parseCSV(txt, customIdxMap = null) {
  ...
  return lines.slice(1).map((line, i) => {
    ...
    return {
      ...
      leadLevel:        get(idxMap.leadLevelAlias ?? -1) || get(idxMap.leadLevel ?? -1) || null,
      beneficiary:      get(idxMap.beneficiary ?? -1) || null,
      reqCoverageRange: get(idxMap.reqCoverageRange ?? -1) || null,
      homeValue:        get(idxMap.homeValue ?? -1) || null,
      householdIncome:  get(idxMap.householdIncome ?? -1) || null,
      requestedCoverage: get(idxMap.reqCoverage ?? -1) || null,
      spouseName:       get(idxMap.spouseName ?? -1) || null,
      spouseAge:        get(idxMap.spouseAge ?? -1) || null,
      spouseDob:        get(idxMap.spouseDob ?? -1) || null,
      dependents:       get(idxMap.dependents ?? -1) || null,
      cellPhone:        get(idxMap.cellPhone ?? -1) || null,
      homePhone:        get(idxMap.homePhone ?? -1) || null,
      workPhone:        get(idxMap.workPhone ?? -1) || null,
      inFunnel: true,
    };
  }).filter(l => l && l.phone);
}
